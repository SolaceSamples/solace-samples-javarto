 ...
 *
 * NOTE: Ensure Subscription Manager is enabled in the Client Username.
 * 
...
public class SubscribeOnBehalfOfClient extends AbstractSample {
	...
	private Topic topicForByteBuffer = Solclient.Allocator
			.newTopic(SampleUtils.SAMPLE_TOPIC + 1);
	...
	protected void run(String[] args, SessionConfiguration config, Level logLevel)
		...
		ByteBuffer clientNameByteBuffer = ByteBuffer.allocateDirect(SolEnum.SolConstants.MAX_CLIENT_NAME_SIZE);
		clientNameByteBuffer.clear();
		clientSessionClientName.getAsBytes(clientNameByteBuffer).flip();
		...
		ByteBuffer topicByteBuffer = ByteBuffer.allocateDirect(SolEnum.SolConstants.MAX_TOPIC_SIZE);
		topicByteBuffer.clear();
		topicByteBuffer.put((SampleUtils.SAMPLE_TOPIC + 1).getBytes(StandardCharsets.UTF_8));
		topicByteBuffer.flip();

		print("Adding another subscription using GC free APIs [" + (SampleUtils.SAMPLE_TOPIC + 1)
				+ "] by Subscription Manager [" + managerSessionCLIENT_NAMEStr
				+ "] on behalf Subscription Client ["
				+ clientSessionClientName.getName() + "]");

		rc = managerSessionHandle.subscribeOnBehalfOfClient(clientNameByteBuffer, topicByteBuffer,
				SolEnum.SubscribeFlags.WAIT_FOR_CONFIRM, 0);
		assertReturnCode("managerSessionHandle.subscribeOnBehalfOfClient ", rc,
				SolEnum.ReturnCode.OK);
		...
		common_publishMessage(managerSessionHandle, txMessageHandle, topicForByteBuffer,
				SolEnum.MessageDeliveryMode.DIRECT);
		...
		if (countingMessageReceiveCallbackForClientSession.getMessageCount() != 2) {
